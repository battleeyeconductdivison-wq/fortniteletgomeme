...
-- Library element functions
    function library:window(properties)
        ...
        local items = cfg.items; do
         ...
         -- Use identifyexecutor exactly like the docs
local executorName, executorVersion = "Unknown", "Unknown"

if identifyexecutor then
    executorName, executorVersion = identifyexecutor()  -- docs example: local exec_name, exec_version = identifyexecutor()
end

-- Create the TextLabel with executor name only
...
library:apply_theme(items["other_info"], "accent", "TextColor3");
    ...
    function library:init_config(window) 
        ...
        section:button({
            ...
            callback = function()
                local folderPath = library.directory .. "/configs"
                
                -- Ensure folder exists
                if not isfolder then
                    notify("Honey", "Your executor does not support folder operations", 4)
                    return
                end
                ...
                local function getExecutorName()
                    -- Volcano Detection
                    if getgenv().volcano or _G.volcano or volcano then
                        return "Volcano"
                    end
                    
                    -- Xeno Detection (multiple methods based on research)
                    if getgenv().xeno or _G.xeno or xeno or getgenv().XENO_LOADED then
                        return "Xeno"
                    end
                    
                    -- Alternative Xeno detection via MessagingService
                    pcall(function()
                        if game:GetService("MessagingService") and not game:GetService("RunService"):IsStudio() then
                            return "Xeno"
                        end
                    end)
                    
                    -- Solara Detection (multiple methods)
                    if getgenv().solara or _G.solara or solara or getgenv().SOLARA_LOADED then
                        return "Solara"
                    end
                    
                    -- Wave Detection (multiple methods)
                    if getgenv().wave or _G.wave or wave or getgenv().WAVE_LOADED then
                        return "Wave"
                    end
                    
                    -- Velocity Detection
                    if getgenv().velocity or _G.velocity or velocity or getgenv().VELOCITY_LOADED then
                        return "Velocity"
                    end
                    
                    -- Swift Detection
                    if getgenv().swift or _G.swift or swift or getgenv().SWIFT_LOADED then
                        return "Swift"
                    end
                    
                    -- Potassium Detection
                    if getgenv().potassium or _G.potassium or potassium or getgenv().POTASSIUM_LOADED then
                        return "Potassium"
                    end
                    
                    -- Bunni.lol Detection
                    if getgenv().bunni or _G.bunni or bunni or getgenv().BUNNI_LOADED then
                        return "Bunni"
                    end
                    
                    -- Nucleus Detection
                    if getgenv().nucleus or _G.nucleus or nucleus or getgenv().NUCLEUS_LOADED then
                        return "Nucleus"
                    end
                    
                    -- Synapse X/Z Detection
                    ...
                    -- Script-Ware V2 Detection
                    if getgenv().scriptware then
                        return "ScriptWare"
                    end
                    
                    -- Oxygen U Detection
                    if getgenv().oxygen then
                        return "OxygenU"
                    end
                    
                    -- Electron Detection
                    if getgenv().electron then
                        return "Electron"
                    end
                    
                    -- Calamari Detection
                    if getgenv().calamari then
                        return "Calamari"
                    end
                    ...
                    -- Trigon Detection
                    if getgenv().trigon then
                        return "Trigon"
                    end
                    
                    -- Valyse Detection
                    if getgenv().valyse then
                        return "Valyse"
                    end
                    
                    -- Furk Detection
                    if getgenv().furk then
                        return "Furk"
                    end
                    
                    -- Arceus Detection
                    if getgenv().arceus then
                        return "Arceus"
                    end
                    
                    -- Hydrogen Detection
                    if getgenv().hydrogen then
                        return "Hydrogen"
                    end
                    
                    -- Nihon Detection
                    if getgenv().nihon then
                        return "Nihon"
                    end
                    
                    -- Temple Detection
                    if getgenv().temple then
                        return "Temple"
                    end
                    
                    -- Codex Detection
                    if getgenv().codex then
                        return "Codex"
                    end
                    
                    -- Vega X Detection
                    if getgenv().vegax then
                        return "VegaX"
                    end
                    
                    -- Proxo Detection
                    if getgenv().proxo then
                        return "Proxo"
                    end
                    
                    -- Zorara Detection
                    if getgenv().zorara then
                        return "Zorara"
                    end
                    
                    -- Comet Detection
                    if getgenv().comet then
                        return "Comet"
                    end
                    
                    -- Evon Detection
                    if getgenv().evon then
                        return "Evon"
                    end
                    
                    -- Unsupported/Unknown Detection
                ...
                -- Function to try opening folder
                local function tryOpenFolder()
                    -- Method 1: os.execute with start (Windows)
                    if not success then
                        pcall(function()
                            os.execute('start "" "' .. folderPath .. '"')
                            success = true
                        end)
                    end
                    
                    -- Method 2: os.execute with explorer (Windows)
                    if not success then
                        pcall(function()
                    ...
                    -- Method 3: os.execute with open (macOS)
                    if not success then
                        pcall(function()
                    ...
                    -- Method 4: os.execute with xdg-open (Linux)
                    if not success then
                        pcall(function()
                            os.execute('xdg-open "' .. folderPath .. '"')
                            success = true
                        end)
                    end
                    
                    -- Method 5: Synapse specific
                    if not success and syn and syn.request then
                        pcall(function()
                            syn.request({
                                Url = "file:///" .. folderPath:gsub("\\", "/"),
                                Method = "GET"
                            })
                            success = true
                        end)
                    ...
                    -- Method 6: Shell execute for some executors
                    if not success and shell and shell.execute then
                        pcall(function()
                            shell.execute('explorer "' .. folderPath .. '"')
                            success = true
                        end)
                    end
                end
                ...
                if success then
                    library.notifications:create_notification({
                        name = "Honey",
                        info = "Opened Config Folder: " .. folderPath,
                        lifetime = 5
                    })
                else
                    library.notifications:create_notification({
                        name = "Honey",
                        info = executorName .. " does not support opening folders",
                        lifetime = 5
                    })
                end
            end
