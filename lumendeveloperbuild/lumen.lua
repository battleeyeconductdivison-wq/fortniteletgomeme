...
local LocalGameTab, MiscTab, TeleportTab = window:tab({name = "General", tabs = {"Main","Misc","Teleports"}})
...
spawn(function()
    while true do
        ServerLabel:UpdateInfo("Players: "..#Players:GetPlayers().." / "..Players.MaxPlayers)
        wait(1)
    end
end)
...
MainSection:toggle({
    ...
    default = false,
    callback = function(state)
        local courtFolder = Workspace:FindFirstChild("Courts")
        if not courtFolder then return end

        local court = courtFolder:FindFirstChild("1v1 Court_4")
        if not court then return end

        local outOfBoundsModel = court:FindFirstChild("OutOfBounds")
        if not outOfBoundsModel then return end

        if state then
            -- Save the parts before removing
            savedParts = {}
            for _, part in pairs(outOfBoundsModel:GetChildren()) do
                savedParts[#savedParts + 1] = part:Clone()
                part:Destroy()
            end
        else
            -- Restore parts
            for _, part in pairs(savedParts) do
                part.Parent = outOfBoundsModel
            end
            savedParts = {}
        end
    end
